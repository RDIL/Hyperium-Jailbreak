...
public class Installer {
    public static void main(String[] args) {
        try {
            ...
            e.printStackTrace();
            final String str = InstallerUtils.getExceptionStackTrace(e).replace("\t", "  ");
    ...
    private static void updateLauncherJson(final File dotMinecraft) throws IOException {
        ...
        if (System.getProperty("java.version").startsWith("1.8") && System.getProperty("sun.arch.data.model", "").equalsIgnoreCase("64")) {
            final File jrePath = new File(System.getProperty("java.home"), "bin" + File.separator + "java" + (InstallerUtils.getOS() == InstallerUtils.OSType.Windows ? "w.exe" : ""));
            if (jrePath.exists()) {
                hyperiumJson.put("javaDir", jrePath.getAbsolutePath());
            } else {
                System.out.println("Local JRE path does not exist, path = " + jrePath.getAbsolutePath());
            }
        }

        profiles.put(installedUUID, hyperiumJson);
    ...
    private static void showMessageVersionNotFound() {
        InstallerUtils.showErrorMessage("Minecraft 1.8.9 not found.\nYou need to start the game with vanilla 1.8.9 at least once before running this!");
    }
}
