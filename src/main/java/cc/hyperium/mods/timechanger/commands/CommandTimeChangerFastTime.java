...
public class CommandTimeChangerFastTime implements BaseCommand {
    ...
    public void onExecute(String[] args) throws CommandException {
        if (args.length == 0) throw new CommandUsageException();

        final double multiplier = NumberUtils.toDouble(args[0], -1.0);

        if (multiplier < 0.0) {
            GeneralChatHandler.instance().sendMessage(ChatColor.RED + "Invalid multiplier!", false);
            return;
        }

        this.mod.setTimeType(TimeType.FAST);
        this.mod.setFastTimeMultiplier(multiplier);
        GeneralChatHandler.instance().sendMessage(ChatColor.GREEN + "Time set to fast (" + multiplier + ").", false);
    }
}
