...
public class TabToggleEventListener {
    ...
    public void onKeyPress(KeypressEvent e) {
        if (!Minecraft.getMinecraft().gameSettings.keyBindPlayerList.isPressed()) return;
    ...
    public void onRenderHUD(RenderHUDEvent e) {
        ...
        if (TabToggleSettings.ENABLED && TabToggleSettings.TAB_TOGGLED && !Minecraft.getMinecraft().gameSettings.keyBindPlayerList.isKeyDown()) {
            if (Minecraft.getMinecraft().isIntegratedServerRunning() && Minecraft.getMinecraft().thePlayer.sendQueue.getPlayerInfoMap().size() <= 1 && scoreObjective == null) {
